...
export const geppetto-digitalTheme = {
  colors: {
...
export const BrandPreview = ({ config }: { config: any }) => {
  const theme = config?.useGeppetto DigitalTheme ? geppetto-digitalTheme : {
    colors: {
...
export const applyGeppetto DigitalDefaults = (config: any) => {
  if (!config.brand) {
    config.brand = {};
  }
  
  // Se não houver cores customizadas, usa Geppetto Digital
  if (!config.brand.colors || config.useGeppetto DigitalTheme) {
    config.brand.colors = geppetto-digitalTheme.colors;
    config.brand.gradients = geppetto-digitalTheme.gradients;
    config.brand.shadows = geppetto-digitalTheme.shadows;
  }
...
brandCustomizerRoute.post('/api/brand/save', async (c) => {
  try {
    ...
    const finalConfig = applyGeppetto DigitalDefaults(config);
...
brandCustomizerRoute.get('/api/brand/load/:brandId', (c) => {
  ...
  const config = {
    brand: {
      ...
      useGeppetto DigitalTheme: true
    }
  };
  
  return c.json(applyGeppetto DigitalDefaults(config));
});
